...
proc = subprocess.Popen(
    [sys.executable, "-m", "uvicorn", "app.main:app", "--port", "8010", "--app-dir", _backend],
    env=env,
    cwd=_ROOT,
...
try:
    ...
    check(
        "create returns custom_alias",
        created["custom_alias"] == "e2e-check",
        f"custom_alias={created['custom_alias']}",
    )
    check(
        "create returns max_clicks",
        created["max_clicks"] == 5,
        f"max_clicks={created['max_clicks']}",
    )
    check(
        "created_at carries UTC offset",
        created["created_at"].endswith("Z"),
    ...
    check(
        "recent returns custom_alias",
        recent[0]["custom_alias"] == "e2e-check",
    ...
    check(
        "alias cleared", updated["custom_alias"] is None, f"custom_alias={updated['custom_alias']}"
    )
    check(
        "click limit cleared", updated["max_clicks"] is None, f"max_clicks={updated['max_clicks']}"
    )

    code = updated["short_code"]
    check(
        "short_code fell back to generated code",
        code != "e2e-check" and len(code) > 0,
        f"code={code}",
    )
    ...
    check("redirect works", redirect_status in (301, 302, 307), f"status={redirect_status}")

    status, raw = call("GET", f"/api/stats/{code}", headers=AUTH_HEADER)
    stats = json.loads(raw)
    click = stats["recent_clicks"][0]
    check(
        "stats exposes parsed browser", click["browser"] == "Chrome", f"browser={click['browser']}"
    )
    check("stats exposes parsed os", click["os"] == "Windows", f"os={click['os']}")
    check(
        "clicked_at carries UTC offset",
        click["clicked_at"].endswith("Z"),
        f"clicked_at={click['clicked_at']}",
    )
    check("stats total_clicks", stats["total_clicks"] == 1, f"total_clicks={stats['total_clicks']}")

    status, raw = call(
        "GET",
        "/api/stats/%s/export?start_date=2026-01-01T00:00:00Z&end_date=2027-01-01T00:00:00Z" % code,
        headers=AUTH_HEADER,
    )
    header_line = raw.decode().splitlines()[0]
    check(
        "csv export honours date range",
